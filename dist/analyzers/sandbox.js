 ...
 */
function parseMonitorOutput(stdout, stderr, exitCode, duration) {
    const fileAccess = [];
    ...
    for (const line of fileLines) {
        ...
        if (parts.length >= 3) {
            ...
            fileAccess.push({ path, operation: operation, timestamp: Date.now() });
        }
    }
    // Deduplicate honeypot hits
    const uniqueHoneypotHits = honeypotHits.filter((hit, idx, arr) => arr.findIndex(h => h.path === hit.path) === idx);
    return {
        exitCode,
        stdout,
        stderr,
...
function extractSection(text, start, end) {
    ...
    if (startIdx === -1)
        return '';
    if (endIdx === -1)
        return text.slice(startIdx + start.length);
...
function analyzeResults(result, skillPath) {
    ...
    const sensitiveAccess = result.fileAccess.filter(f => SENSITIVE_PATHS.some(p => p.test(f.path)));
    ...
    const suspiciousNetwork = result.networkActivity.filter(n => SUSPICIOUS_DOMAINS.some(d => d.test(n.destination)));
    ...
    if (result.networkActivity.length > 0 && suspiciousNetwork.length === 0) {
        findings.push({
            ...
            location: skillPath,
            evidence: result.networkActivity.slice(0, 10).map(n => `${n.protocol}://${n.destination}:${n.port}`).join('\n'),
            remediation: 'Verify these network destinations are expected for this skill.'
        });
    }
    // High: Suspicious command execution
    const dangerousCommands = result.processExecution.filter(p => /curl|wget|nc|netcat|bash.*-c|sh.*-c|python.*-c|eval|base64/i.test(p.command + ' ' + p.args.join(' ')));
 ...
 */
export function createSandboxAnalyzer() {
    return {
        async analyze(skillPath) {
            ...
            try {
                ...
                return [{
                        ...
                        description: `Error during sandbox execution: ${error.message}`,
                        location: skillPath,
