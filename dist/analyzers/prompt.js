 ...
 */
export function detectInvisibleUnicode(text) {
    const findings = [];
 ...
 */
export function containsInvisibleUnicode(text) {
 ...
 */
export function detectHiddenWhitespace(text) {
    const findings = [];
 ...
 */
export function hasHiddenWhitespaceBlocks(text) {
 ...
 */
export function hasHomoglyphObfuscation(text) {
    return detectHomoglyphs(text).length > 0;
}
/**
 * Detect RTL (right-to-left) override attacks
 */
export function detectRtlOverride(text) {
    const findings = [];
    const rtlChars = ['\u202E', '\u202D', '\u202B', '\u202A', '\u2067', '\u2066'];
    for (let i = 0; i < text.length; i++) {
 ...
 */
export function hasRtlOverride(text) {
 ...
 */
function getRemediation(patternId) {
    const remediations = {
 ...
 */
function extractCapabilities(frontmatter) {
    const capabilities = [];
...
function analyzeSemantics(doc, filePath) {
    ...
    if (simpleIndicators.some(s => description.includes(s))) {
        const dangerous = capabilities.filter(c => dangerousCapabilities.some(d => c.toLowerCase().includes(d)));
...
export class PromptAnalyzer {
                ...
                if (pattern.pattern) {
                    ...
                    try {
                        ...
                        while ((match = regex.exec(text)) !== null) {
                            const lineNum = getLineNumber(doc.raw, name === 'frontmatter' ? match.index : doc.raw.indexOf(doc.body) + match.index);
     ...
     */
    async analyze(skillPath, skillDoc) {
        const allFindings = [];
...
export function createPromptAnalyzer() {
