 ...
 */
export async function createSnapshot(skillPath: string): Promise<SkillSnapshot> {
 ...
 * Load skill history
 */
async function loadHistory(skillName: string): Promise<SkillHistory | null> {
  try {
...
async function saveHistory(history: SkillHistory): Promise<void> {
  ...
  await writeFile(
    join(HISTORY_DIR, `${history.skillName}.json`),
...
function compareSnapshots(
  oldSnap: SkillSnapshot,
  newSnap: SkillSnapshot,
  skillName: string
): Finding[] {
  ...
  if (newCaps.length > 0) {
    const severity: Severity = newCaps.some(c => 
      ['credential_access', 'exec', 'network_access'].includes(c)
    ) ? 'high' : 'medium';
 ...
 */
export async function analyzeDiff(skillPath: string): Promise<{
  findings: Finding[];
  isNew: boolean;
 ...
 * Get skill history
 */
export async function getHistory(skillName: string): Promise<SkillHistory | null> {
 ...
 * Clear skill history
 */
export async function clearHistory(skillName: string): Promise<void> {
