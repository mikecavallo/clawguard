...
import { 
  loadConfig, saveConfig, isConfigured, PROVIDERS, 
  type ClawGuardConfig, CONFIG_FILE 
} from './config.js';
...
function createPrompt(): PromptInterface {
  ...
  return {
    ...
    askPassword: (question: string) => new Promise((resolve) => {
 ...
 */
async function wizardSelectProvider(prompt: PromptInterface): Promise<'anthropic' | 'openai' | 'local' | null> {
  clearScreen();
  console.log(BANNER);
 ...
 */
async function wizardSelectModel(prompt: PromptInterface, provider: 'anthropic' | 'openai' | 'local'): Promise<string> {
  clearScreen();
  console.log(BANNER);
  console.log(`${DIM}  Revolutionary Security Scanner for AI Agent Skills${RESET}`);
  console.log('');
...
async function wizardApiKey(
  ...
  if (provider === 'local') {
    ...
    // Test connection
    console.log('');
    const result = await withSpinner('Testing Ollama connection...', () => 
      testApiConnection('local', model, undefined, baseUrl)
    );
  ...
  // Test API connection
  console.log('');
  const result = await withSpinner('Testing API connection...', () =>
    testApiConnection(provider, model, apiKey)
  );
 ...
 */
async function wizardComplete(prompt: PromptInterface, config: ClawGuardConfig): Promise<void> {
  clearScreen();
  console.log(BANNER);
  console.log(`${DIM}  Revolutionary Security Scanner for AI Agent Skills${RESET}`);
  console.log('');
 ...
 */
async function runConfigWizard(prompt: PromptInterface): Promise<boolean> {
 ...
 */
async function showDashboard(prompt: PromptInterface): Promise<string> {
  clearScreen();
...
async function scanSkill(
  ...
  const result = await withSpinner('Running security analysis', () =>
    orchestrator.scan({
      path: skillPath,
 ...
 */
async function main(options?: { configOnly?: boolean }): Promise<void> {
...
export async function runTui(options?: { configOnly?: boolean }): Promise<void> {
  return main(options);
}
