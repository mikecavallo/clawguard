...
import type { Finding, StaticAnalyzer, PatternDef } from '../types.js';
...
async function loadPatterns(): Promise<CompiledPattern[]> {
  ...
  try {
    ...
    const parsed = parseYaml(content) as { patterns: PatternDef[] };
    
    return parsed.patterns.map((p): CompiledPattern => ({
      id: p.id,
      name: (p as unknown as { name: string }).name || p.id,
 ...
 */
function detectObfuscation(content: string, filename: string): Finding[] {
  const findings: Finding[] = [];
  const lines = content.split('\n');
...
export function createStaticAnalyzer(): StaticAnalyzer {
  let patterns: CompiledPattern[] | null = null;
