...
): { original: string; distance: number } | null {
 ...
 */
function isHomoglyphVariant(name: string, target: string): boolean {
  if (name.length !== target.length) return false;
  
  let differences = 0;
  for (let i = 0; i < name.length; i++) {
    if (name[i] === target[i]) continue;
 ...
 */
function isAdjacentKeyTypo(name: string, target: string): boolean {
  if (name.length !== target.length) return false;
 ...
 */
function isVowelDropped(name: string, target: string): boolean {
 ...
 */
function isCharacterOmission(name: string, target: string): boolean {
  if (name.length !== target.length - 1) return false;
 ...
 */
function isCharacterInsertion(name: string, target: string): boolean {
  if (name.length !== target.length + 1) return false;
 ...
 */
function isCharacterSwap(name: string, target: string): boolean {
  if (name.length !== target.length) return false;
 ...
 */
export function checkTyposquatPatterns(
  name: string,
  popularPackages: string[]
