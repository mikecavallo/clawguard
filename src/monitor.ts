...
export interface MonitorConfig {
  enabled: boolean;
  watchPaths: string[];
  scanIntervalMs: number;  // How often to rescan
...
class SkillMonitor extends EventEmitter {
  ...
  private setupWatcher(skillPath: string): void {
    try {
  ...
  private async scanPath(skillPath: string): Promise<void> {
  ...
  private async alert(params: Omit<Alert, 'id' | 'timestamp'>): Promise<void> {
    const alert: Alert = {
...
// Singleton instance
let monitorInstance: SkillMonitor | null = null;

export function getMonitor(): SkillMonitor {
  if (!monitorInstance) {
