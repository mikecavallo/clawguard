...
async function runScan(
  ...
  const outputFormat = (options.output || 'md') as OutputFormat;
...
async function runFullScan(
  skillPath: string,
  options: { output?: string; apiKey?: string; verbose?: boolean }
): Promise<void> {
  console.log('╔════════════════════════════════════════════════════════════════╗');
...
async function runGate(
  skillPath: string,
  options: { sandbox?: boolean; allowHigh?: boolean; json?: boolean }
): Promise<void> {
  const config = await loadConfig();
  const orchestrator = createOrchestrator();
...
export function register(program: Command): void {
  ...
  program
    ...
    .action(async (skillPath: string, options) => {
      let fetchResult: Awaited<ReturnType<typeof fetchSkillFromUrl>> | null = null;
  ...
  program
    ...
    .action(async (skillPath: string, options) => {
      ...
      try {
        ...
        console.error('Gate error:', (error as Error).message);
        process.exit(1);
      } finally {
        if (fetchResult) {
          await fetchResult.cleanup();
