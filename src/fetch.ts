 ...
 */
async function fetchTarGz(url: string, tempDir: string): Promise<FetchResult> {
 ...
 */
async function fetchZip(url: string, tempDir: string): Promise<FetchResult> {
  console.log(`📥 Downloading ${basename(url)}...`);
 ...
 */
async function fetchGitRepo(url: string, tempDir: string): Promise<FetchResult> {
